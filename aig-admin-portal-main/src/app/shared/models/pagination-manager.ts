export class PaginationManager {
 ...
 public setPageData(data: any) {
  this.first = data.firstPage ?? data.first ?? false;
  this.last = data.lastPage ?? data.last ?? false;
  this.numberOfElements_currentPage = data.numberOfElements;
  this.totalElements = data.totalElements;
  this.totalPages = data.totalPages;
  this.pageNumber = data.pageNumber ?? data.pageable?.pageNumber ?? 0;
  this.pageSize = data.pageSize ?? data.pageable?.pageSize ?? 0;
